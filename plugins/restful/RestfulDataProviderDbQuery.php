...
abstract class RestfulDataProviderDbQuery extends \RestfulBase implements \RestfulDataProviderDbQueryInterface, \RestfulDataProviderInterface {
  ...
  public function getIdColumn() {
    ...
    // If idColumn is not an array return an array with the single column.
    if (!is_array($this->idColumn)) {
      return array($this->idColumn);
    }
   ...
   * where the strings are divided by the delimiter "::"
   *
   * @param array $ids
   * @param int $column
   *  0-N Zero indexed
