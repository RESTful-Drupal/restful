...
class RestfulGetHandlersTestCase extends DrupalWebTestCase {
  ...
  function testGetHandlers() {
    ...
    $this->assertEqual(get_class($handler), 'RestfulTestArticlesResource');
    ...
    $this->assertEqual(get_class($handler), 'RestfulTestArticlesResource__1_1');
    $result = $handler->get($node1->nid);
    $this->assertFalse(!empty($result['self']), '"self" property does not appear in minor version 1.');
  }
